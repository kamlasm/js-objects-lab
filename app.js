...
*/
game.party.push(pokemon[24], pokemon[145], pokemon[64]);
...
game.catchPokemon = (pokemonName) => {
    const caughtPokemon = pokemonName;
    const formatCaughtPokemon = caughtPokemon.charAt(0).toUpperCase() + caughtPokemon.slice(1).toLowerCase();
    
    if ((pokemon.find(({name}) => name === formatCaughtPokemon))) {
        const pokemonObject = pokemon.find(({name}) => name === formatCaughtPokemon);
        if (game.items[1].quantity < 1) {
            return "Not enough pokeballs!"
        } else {
            if (game.party.length < 6) {
...
const pokemonByType = pokemon.reduce((acc, poke) => {
    if (acc[poke.type]) {
